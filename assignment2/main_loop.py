...
def cs_main(sps_rating_matrix, n_bands, projections_per_band, threshold):
    ...
    pairs = find_candidate_pairs(signatures, n_bands, projections_per_band)
    pairs = find_signature_pairs(pairs, signatures, threshold)
    pairs = find_cosine_pairs(pairs, sps_rating_matrix, threshold)

    n_cosine_pairs = pairs.shape[0]
    print(f'Number of pairs with cos_sim > {threshold}:', n_cosine_pairs)
    
    return pairs


def main_loop(data, measure_idx, measure, seed, n_bands, rows_per_band, threshold, save_file):
    np.random.seed(seed)
    
    start_time = time.time()
    pairs = measure(data, n_bands, rows_per_band, threshold)
    run_time = time.time() - start_time
    
    n_pairs = len(pairs)
    with open(save_file, 'a') as run_file:
        run_file.write(f'{measure_idx}, {seed}, {n_bands}, {rows_per_band}, {n_pairs}, {run_time}')
...
if __name__ == '__main__':
    ...
    save_file = 'runs_log.txt'
    
    # the first value of the parameters is the number of band
    # and the second value is the number of permutations.
    js_settings = [[10,8],[30,5],[20,5],[8,10],[10,5]]
    cs_settings = [[5,15],[10,15],[15,15],[10,12],[10,18]]
    random_seeds = [5,11,20,30,42]
    measure_funcs = [js_main, cs_main, cs_main]
    all_settings = [js_settings, cs_settings, cs_settings]
    discrete = [False, False, True]
    threshold = [0.5, 0.73, 0.73]
    
    for seed in random_seeds:
        for i, measure in enumerate(measure_funcs):
            for n_bands, rows_per_band in all_settings[i]:
                data = load_data_into_matrix(data_file, n_users, n_movies, n_ratings, discrete[i])
                main_loop(data, i, measure, seed, n_bands, rows_per_band, threshold[i], save_file)
